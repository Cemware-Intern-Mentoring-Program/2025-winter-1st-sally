...
단점
“정식 문서”보다는 “테스트용 스펙”에 가깝고
Swagger처럼 코드에서 자동 생성되는 문서는 아님


**👽 코드 기반 자동 문서화 도구 (Swagger / OpenAPI 계열)**
📌 Swagger / OpenAPI
OpenAPI:
REST API 명세를 위한 표준 포맷(스펙) 이름
Swagger:
이 OpenAPI 스펙을 만들고 보여주는 대표적인 툴/라이브러리 이름
Spring에서 많이 쓰는 방식:
springdoc-openapi 라이브러리 추가
Controller 메소드에 간단한 어노테이션 달기
→ 자동으로 API 문서 + 웹 UI 생성
(예: /swagger-ui.html, /swagger-ui/index.html로 접속)
장점
코드와 문서가 같이 관리됨
(= 코드 바꾸면 문서도 자동 반영)
