...
public class GroupService {
    ...
    public Long createGroup(Long userId, String name, String description) {
        ...
        // 2) Group 엔티티 생성
        Group group = new Group(user, name, description);
    ...
    public GroupWithTasks getGroupWithTasks(Long groupId) {
        // 1) 그룹 조회
        Group group = groupRepository.findById(groupId)
                .orElseThrow(() -> new IllegalArgumentException("그룹이 존재하지 않습니다. id=" + groupId));
        ...
        // 3) 둘을 한 번에 담아서 반환
        return new GroupWithTasks(group, tasks);
    }

    //그룹 + 하위 할 일 목록을 한 번에 전달하기 위한 간단한 DTO
    public static class GroupWithTasks {
        private final Group group;
        private final List<Task> tasks;

        public GroupWithTasks(Group group, List<Task> tasks) {
            this.group = group;
            this.tasks = tasks;
        }
